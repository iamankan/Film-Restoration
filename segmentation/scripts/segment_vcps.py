...
def draw_graph_with_endpoints(G, output_path='graph.jpg'):
...
def segment(volpkg_path: Path, volume_id: str, output_dir: Path, slice_name: str, threshold: int, min_connected_points: int, connectivity:int, gaussian_kernel: int,
    ...
    if len(components) > 0:
        for component_id in components:
            ...
            pruned, shortest_path = prune_skeleton_with_graph(thinned_mask*255, min_branch_length=30)

            for i, p in enumerate(shortest_path):
                frac = i/len(shortest_path)
                cv2.circle(dummy_image, (p[1], p[0]), 2, color=(0, int((1-frac)*255), int(frac*255)), thickness=2)
            
            cv2.line(dummy_image, (shortest_path[0][1], shortest_path[0][0]), (shortest_path[1][1], shortest_path[1][0]), color=(255,0,0), thickness=3)
            
            if output_dir:
                cv2.imwrite(f'{component_dir}/pruned_thinned_mask_component_{component_id}.jpg', pruned)
                cv2.imwrite(f'{component_dir}/ordered_ps_component_{component_id}.jpg', dummy_image)
