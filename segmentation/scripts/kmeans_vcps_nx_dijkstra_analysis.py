...
def thin(volpkg_dir: Path, volume: str, film_slice: str, original_image: np.array, clustered: np.array, save_at: str, cluster_id: int, 
    ...
    for idx in range(1, num_labels):  # skip background
        ...
        # if len(pointset[0])>0:
            ...
            # seg_id = f'{get_date()}_kmeans_thin_dijkstra_k_{gaussian_kernel}'
            # if write_vcps:
    ...
    if save_at:
        ...
        cv2.imwrite(f'{save_at_cluster}/dijkstra_mask.jpg', img=binary_segmentation_mask)
        with open("/localdisk0/thesis-images/nx-vol/stats.txt","a") as f:
            f.write(f"Dijkstra,{slice_id},{gaussian_kernel},{num_seg_points},{total_segments}\n")
        # total_segments
