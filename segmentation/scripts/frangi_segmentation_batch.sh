...
FRANGI_FLAG="" # Starts empty

print_help() {
    echo "Usage: $0 --volpkg <path> --output-dir <path> [--write-vcps] [--use-frangi] [--help]"
    echo ""
    echo "Required:"
    ...
    echo "  --use-frangi       Enable the Frangi vesselness filter prior stage"
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --use-frangi) FRANGI_FLAG="--use-frangi" ;; # Capture your new flag
...
if [ -z "$VOLPKG" ] || [ -z "$OUTPUT_DIR_BASE" ]  || [ -z "$VOLUME_ID" ]; then
    echo "Error: --volpkg, --output-dir, and --volume-id are required."
    echo ""
    print_help
    exit 1
fi
...
echo "Use Frangi: $(( [ -n "$FRANGI_FLAG" ] && echo "true" ) || echo "false")"
echo "--------------------------------------------------------"
...
for (( i=$START_NUM; i<=$END_NUM; i+=$STEP_SIZE )); do
...

    {
        ...
        echo "USE_FRANGI=$([ -n "$FRANGI_FLAG" ] && echo "true" || echo "false")"
    ...
    # Initialize standard array arguments
    CMD=(python3 segmentation/scripts/frangi_segmentation.py
        --volpkg "$VOLPKG"
        --volume "$VOLUME_ID"
    ...
    # Dynamically inject toggles without trailing empty entries
    if [ -n "$FRANGI_FLAG" ]; then
        CMD+=("$FRANGI_FLAG")
    fi
    ...
    echo "✅ Finished ${VOLUME_ID} (${CURRENT_SLICE}) in ${duration} seconds."
    echo ""
done
