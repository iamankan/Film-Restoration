...
def remove_skeleton_bridges(G, max_bridge_length=15):
    print(f'Removing Y-junctions...........')
    """
    1. Detects all Y-junctions (degree == 3).
    2. Disconnects them entirely from the graph to isolate all lines.
    3. Deletes any standalone segment shorter than max_bridge_length.
    4. Reconnects the valid continuous paths.
    """
    # 1. Detect the Y-junctions (and any complex intersections)
    junctions = {node for node, degree in G.degree() if degree >= 3}

    print(f'Number of Y-junctions: {len(junctions)}')
    
    if not junctions:
        return G # Nothing to do if there are no junctions
        
    # Keep track of the exact edges that connected to these junctions
    original_junction_edges = []
    for j in junctions:
        for neighbor in G.neighbors(j):
            original_junction_edges.append((j, neighbor))
            
    # 2. Disconnect the Y-junctions by removing the junction nodes entirely
    G_broken = G.copy()
    G_broken.remove_nodes_from(junctions)
    
    # # 3. Find all isolated segments now that intersections are gone
    # segments = list(nx.connected_components(G_broken))
    
    # nodes_to_permanently_delete = []
    
    # # Identify which isolated lines are just noise/artifacts based on length
    # for segment in segments:
    #     if len(segment) <= max_bridge_length:
    #         # This segment was a short cross-bar bridge or an artifact stub!
    #         nodes_to_permanently_delete.extend(segment)
            
    # # Remove the bad bridge pixels from the master graph
    # G.remove_nodes_from(nodes_to_permanently_delete)
    
    # # 4. Clean up any remaining edges that used to connect to deleted bridge parts
    # # Re-verify that both ends of the original connection still exist before restoring
    # for u, v in original_junction_edges:
    #     if not G.has_node(u) or not G.has_node(v):
    #         if G.has_edge(u, v):
    #             G.remove_edge(u, v)
                
    # # 5. Sweep for single isolated floating pixels left behind
...
def thin(volpkg_dir: Path, volume: str, film_slice: str, original_image: np.array, clustered: np.array, save_at: str, cluster_id: int, 
    ...
    for idx in range(1, num_labels):  # Skip background
        ...
        # This returns the broken graph containing ONLY the long, valid structural segments
        graph_broken = remove_skeleton_bridges(graph, max_bridge_length=15)
        
        # Draw these pristine, isolated lines onto our global clean canvas
        for node in graph_broken.nodes():
            clean_skeleton[node[0], node[1]] = 255

    # print(f"Total components: {num_labels} (including background).")
    # Save out the intermediate clean skeleton so you can verify the change visually
    if save_at:
    ...
    # Initialize global binary segmentation mask for down-stream tracing results
    binary_segmentation_mask = np.zeros_like(original_image[:, :, 0])

    # =========================================================================
    # PASS 2: RE-CALCULATE COMPONENTS ON CLEAN SKELETON & RUN PATHFINDER
    # =========================================================================
    # We execute connectedComponents AGAIN. Now, every single isolated spiral arm 
    # is recognized as its own completely separate index!
    num_clean_labels, clean_labels = cv2.connectedComponents(clean_skeleton, connectivity=8)
    

    for idx in range(1, num_clean_labels):  # Skip background
        component_mask_clean = (clean_labels == idx).astype(np.uint8)
        
        # Rebuild a perfectly clean, line-like graph for this specific isolated ring segment
        ...
        # Now your endpoint finder will only find 2 true endpoints (the ends of the ring segment)
        ...
        # Use the isolated component mask to track pathing weights smoothly
        weighted_graph = skeleton_to_weighted_graph(skeleton=component_mask_clean, center_point=center_point, image=clustered, 
        ...
        for sp in shortestpath:
            cv2.circle(binary_segmentation_mask, (sp[1], sp[0]), mask_thickness, 255, mask_thickness)
